---
description: Agent Orchestrator - coordinates sub-agents, never does work inline
mode: primary 
permission:
  task:
    "*": deny
...
Bind this to the dedicated `kyubi` agent or rule only. Do NOT apply it to executor phase agents such as `sdd-apply` or `sdd-verify`.
